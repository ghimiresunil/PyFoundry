...
Report bugs at https://github.com/ghimiresunil/pyfoundry/issues.
...
pyfoundry could always use more documentation, whether as part of the
...
https://github.com/ghimiresunil/pyfoundry/issues.
...
Ready to contribute? Here's how to set up **pyfoundry** for local development. Please
note this documentation assumes you already have ``uv`` and ``git`` installed and
ready to go.

#. Clone the **pyfoundry** repo locally:

   .. code-block:: console

        $ git clone git@github.com:ghimiresunil/pyfoundry.git
        $ cd pyfoundry
...
#. The pull request should work for all supported Python versions. Check
   https://github.com/ghimiresunil/pyfoundry/actions
...
1. First, ensure the `__version__` in `pyfoundry/__init__.py` is updated to the new version.
